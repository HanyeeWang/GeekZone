...
public class DiskLruCache {
    ...
    public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
        ...
        if (cacheDir.isDirectory() && cacheDir.canWrite() && com.hanyee.androidlib.cache.image.ImageUtils.getUsableSpace(cacheDir) > maxByteSize) {
        ...
        synchronized (mLinkedHashMap) {
            if (mLinkedHashMap.get(key) == null) {
                try {
                    ...
                    LogUtils.e("Error in put: " + e.getMessage());
    ...
    /**
     * Removes disk cache entry from this instance cache dir by key
     */
    public void remove(String key) {
        synchronized (mLinkedHashMap) {
            String existingFile = mLinkedHashMap.remove(key);
            if (existingFile == null) {
                existingFile = createFilePath(mCacheDir, key);
            }
            File bitmapFile = new File(existingFile);
            if (bitmapFile.exists()) {
                cacheByteSize -= bitmapFile.length();
                cacheByteSize = cacheByteSize < 0 ? 0 : cacheByteSize;
                bitmapFile.delete();
            }
            cacheSize = mLinkedHashMap.size();
        }
    ...
    public static String createFilePath(File cacheDir, String key) {
        try {
            ...
            LogUtils.e("Error in createFilePath - " + e.getMessage());
        }
    ...
    private boolean writeBitmapToFile(Bitmap bitmap, String file) throws IOException, FileNotFoundException {
        ...
        try {
            out = new BufferedOutputStream(new FileOutputStream(file), com.hanyee.androidlib.cache.image.ImageUtils.IO_BUFFER_SIZE);
